...
void SignalEvent::signalHandler(int signo, siginfo_t* siginfo, void* ucontext) {
    ...
    while (true) {
        if (now - last_sample < (u64)_interval) return;
    ...
    if (trace != 0 && _pipe[1] >= 0) {
        StreamSample sample = {OS::micros() / 1000, trace};
...
void SignalEvent::writerLoop() {
    ...
    while (true) {
        ...
        if (bytes == sizeof(sample)) {
            if (attached) {
                Profiler::instance()->writeStreamEvent(out, sample.timestamp, sample.trace);
            }
        } else if (bytes == 0) {
            break;
